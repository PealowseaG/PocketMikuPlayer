...
def play_song():
    ...
    bpmrate = 1.0
    # play midi file
    for msg in mido.MidiFile(midifile[filesel-1]):
        if not q_pas.empty():
            pause = q_pas.get()
        if not q_bpm.empty():
            bpmrate = q_bpm.get()
        # pause (1:pause) or standby
        if pause == 1 and (msg.type == 'note_on'):
            while True:
                if not q_pas.empty():
                    pause = q_pas.get()
                    if pause != 1:
                        break
                # wait for escape from pause
                time.sleep(0.05)
        # main player
            ...
            if swchng[ENTER] == 1 and swnow[ENTER] == 0:
                ...
                for msg in mido.MidiFile(midifile[filesel-1]):
                    if msg.type == 'set_tempo':
                        bpmint = mido.tempo2bpm(msg.tempo)
#                        print(msg) # for debug
                        print('start:', int(bpmint), 'bpm (MIN:', int(mido.tempo2bpm(16777215)), '-MAX:', int(mido.tempo2bpm(1)), ')')  # for debug
                        break
                # display bpm
                display_bpm()
                # display standby
                ...
                t1 = threading.Thread(target = play_song)   # midifile[filesel-1] play
...
#                print(msgtx4)   # for debug
        ...
        # catch end of song
        if not q_ply.empty():
            playstate = q_ply.get() # 0:stop, 1:playing
